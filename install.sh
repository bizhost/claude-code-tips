...
if [[ -n "$CBM_OS" && -n "$CBM_ARCH" ]]; then
  if [[ -x "$HOME/.local/bin/codebase-memory-mcp" ]]; then
    ...
    # curl+tar in one guard: a bad/empty download must not abort the whole
    # install under `set -e` — CBM is best-effort, file-install steps must run.
    if curl -fsSL "$CBM_URL" -o "$CBM_TMP/cbm.tar.gz" && tar -xzf "$CBM_TMP/cbm.tar.gz" -C "$CBM_TMP" 2>/dev/null; then
      if [[ -f "$CBM_TMP/codebase-memory-mcp" ]]; then
        ...
        echo "  ⚠ CBM tarball extracted but binary not found."
      fi
    else
      echo "  ⚠ CBM download failed ($CBM_URL). Install manually later."
    fi
    rm -rf "$CBM_TMP"
  fi
fi
